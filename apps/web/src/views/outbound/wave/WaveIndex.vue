...
const {
  ...
  storageKey: 'wave-management-column-settings-v1',
  preferredKeys: ['code', 'name', 'status', 'creationTime'],
  resolveTitle: (key) => {
    if (key === 'code') return '单号'
...
const columnMap: Record<string, DataTableColumns<RowItem>[number]> = {
  code: { title: createDraggableTitle('code', '单号'), key: 'code', minWidth: 160, sorter: (a, b) => compareSortValue(a.code, b.code) },
...
const columns = computed<DataTableColumns<RowItem>>(() => withResizable(
  columnSettings.value.filter((item) => item.visible).map((item) => columnMap[item.key]).filter((item): item is DataTableColumns<RowItem>[number] => Boolean(item)),
))
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item><n-input v-model:value="keyword" clearable placeholder="请输入波次号/名称" @keyup.enter="handleQuery" /></n-form-item>
    ...
    <template #actions-left>
      <div class="crud-action-main"><n-button type="primary" @click="message.info('待接入新增波次')">新增</n-button></div>
    </template>
    ...
    <template #pager-right>
      <n-pagination v-model:page="pagination.page" v-model:page-size="pagination.pageSize" :item-count="pagination.itemCount" :page-sizes="[10, 20, 50, 100]" show-size-picker @update:page="handlePageChange" @update:page-size="handlePageSizeChange" />
    </template>
  </BaseCrudPage>
</template>