...
const detailColumns = computed<DataTableColumns<miscOutboundOrderApi.MiscOutboundOrderDetailDto>>(() => withResizable([
  {
    title: '仓库编码',
    key: 'warehouseCode',
    minWidth: 120,
...
function getRowKey(row: miscOutboundOrderApi.MiscOutboundOrderDetailDto) {
  return row.id
}
...
async function loadDetail() {
  ...
  loading.value = true
  try {
    detail.value = await miscOutboundOrderApi.get(detailId.value)
  } catch (error) {
    message.error(error instanceof Error ? error.message : '加载其他出库详情失败')
  } finally {
    loading.value = false
  }
}
