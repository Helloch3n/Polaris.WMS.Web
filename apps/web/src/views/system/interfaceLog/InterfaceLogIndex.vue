...
const { showColumnConfig, columnSettings, loadColumnSettings, handleVisibleChange, createDraggableTitle } = useColumnConfig({
  storageKey: 'interface-log-column-settings-v1',
  preferredKeys: ['apiName', 'method', 'status', 'creationTime'],
  resolveTitle: (key) => (key === 'apiName' ? '接口' : key === 'method' ? '方法' : key === 'status' ? '状态' : key === 'creationTime' ? '时间' : key),
})
