...
const { showColumnConfig, columnSettings, loadColumnSettings, handleVisibleChange, createDraggableTitle } = useColumnConfig({
  storageKey: 'pallet-merge-column-settings-v1',
  preferredKeys: ['code', 'name', 'status', 'creationTime'],
  resolveTitle: (key) => (key === 'code' ? '合盘单号' : key === 'name' ? '来源容器' : key === 'status' ? '状态' : key === 'creationTime' ? '创建时间' : key),
})

const columnMap: Record<string, DataTableColumns<RowItem>[number]> = {
  code: { title: createDraggableTitle('code', '合盘单号'), key: 'code', minWidth: 160, sorter: (a, b) => compareSortValue(a.code, b.code) },
  name: { title: createDraggableTitle('name', '来源容器'), key: 'name', minWidth: 180, sorter: (a, b) => compareSortValue(a.name, b.name) },
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item><n-input :value="keyword" clearable placeholder="请输入合盘单号/容器号" @update:value="(value) => (keyword = value)" @keyup.enter="handleQuery" /></n-form-item>
    ...
    <template #actions-left><div class="crud-action-main"><n-button type="primary" @click="message.info('待接入新增合盘')">新增</n-button></div></template>
