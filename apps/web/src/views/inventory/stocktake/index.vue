...
const { showColumnConfig, columnSettings, loadColumnSettings, handleVisibleChange, createDraggableTitle } = useColumnConfig({
  storageKey: 'stocktake-management-column-settings-v1',
  preferredKeys: ['code', 'name', 'status', 'creationTime'],
  resolveTitle: (key) => (key === 'code' ? '盘点单号' : key === 'name' ? '盘点名称' : key === 'status' ? '状态' : key === 'creationTime' ? '创建时间' : key),
})

const columnMap: Record<string, DataTableColumns<RowItem>[number]> = {
  code: { title: createDraggableTitle('code', '盘点单号'), key: 'code', minWidth: 160, sorter: (a, b) => compareSortValue(a.code, b.code) },
  name: { title: createDraggableTitle('name', '盘点名称'), key: 'name', minWidth: 180, sorter: (a, b) => compareSortValue(a.name, b.name) },
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item><n-input :value="keyword" clearable placeholder="请输入盘点单号/名称" @update:value="(value) => (keyword = value)" @keyup.enter="handleQuery" /></n-form-item>
    ...
    <template #actions-left><div class="crud-action-main"><n-button type="primary" @click="message.info('待接入新增盘点')">新增</n-button></div></template>
