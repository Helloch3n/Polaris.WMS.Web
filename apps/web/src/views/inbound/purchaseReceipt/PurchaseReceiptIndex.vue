...
const {
  ...
  storageKey: 'purchase-receipt-column-settings-v1',
  preferredKeys: ['code', 'name', 'status', 'creationTime'],
  resolveTitle: (key) => {
    if (key === 'code') return '收货单号'
    if (key === 'name') return '供应商'
...
const columnMap: Record<string, DataTableColumns<RowItem>[number]> = {
  code: { title: createDraggableTitle('code', '收货单号'), key: 'code', minWidth: 160, sorter: (a, b) => compareSortValue(a.code, b.code) },
  name: { title: createDraggableTitle('name', '供应商'), key: 'name', minWidth: 180, sorter: (a, b) => compareSortValue(a.name, b.name) },
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item><n-input :value="keyword" clearable placeholder="请输入收货单号/供应商" @update:value="(value) => (keyword = value)" @keyup.enter="handleQuery" /></n-form-item>
    ...
    <template #actions-left>
      <div class="crud-action-main"><n-button type="primary" @click="message.info('待接入新增采购收货')">新增</n-button></div>
    </template>
