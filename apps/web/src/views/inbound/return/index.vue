...
const { showColumnConfig, columnSettings, loadColumnSettings, handleVisibleChange, createDraggableTitle } = useColumnConfig({
  ...
  resolveTitle: (key) => (key === 'code' ? '销售退货单号' : key === 'name' ? '客户' : key === 'status' ? '状态' : key === 'creationTime' ? '创建时间' : key),
})

const columnMap: Record<string, DataTableColumns<RowItem>[number]> = {
  code: { title: createDraggableTitle('code', '销售退货单号'), key: 'code', minWidth: 160, sorter: (a, b) => compareSortValue(a.code, b.code) },
  name: { title: createDraggableTitle('name', '客户'), key: 'name', minWidth: 180, sorter: (a, b) => compareSortValue(a.name, b.name) },
  status: {
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item><n-input :value="keyword" clearable placeholder="销售退货单号/客户" @update:value="(value) => (keyword = value)" @keyup.enter="handleQuery" /></n-form-item>
    ...
    <template #actions-left><div class="crud-action-main"><n-button type="primary" @click="message.info('销售退货业务待接入')">新增</n-button></div></template>
