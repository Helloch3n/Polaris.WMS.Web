...
const order = ref<ReviewOrderDto | null>(null)
const loading = ref(false)
const scanCode = ref('')
const scanInput = ref<HTMLInputElement | null>(null)
const exceptionVisible = ref(false)
const exceptionLine = ref<ReviewLineDto | null>(null)
const exceptionType = ref('')
const exceptionRemark = ref('')

const editable = computed(() => order.value != null && ![ReviewStatus.Completed, ReviewStatus.Cancelled].includes(order.value.status as 20 | 30))
const canComplete = computed(() => order.value?.status === ReviewStatus.Reviewing
  && order.value.reviewedLineCount === order.value.totalLineCount
  && order.value.exceptionLineCount === 0)

function lineLabel(status: number) {
  if (status === ReviewLineStatus.Passed) return '已通过'
  if (status === ReviewLineStatus.Exception) return '异常'
  return '待复核'
}

function lineType(status: number) {
  if (status === ReviewLineStatus.Passed) return 'success'
  if (status === ReviewLineStatus.Exception) return 'danger'
  return 'warning'
}
...
async function load() {
  loading.value = true
  try {
    order.value = await getReview(id.value)
  } catch (error: any) {
    showFailToast(error?.message || '加载复核单失败')
  } finally {
    loading.value = false
  }
}
...
async function start() {
  try {
    order.value = await startReview(id.value)
    showSuccessToast('复核已开始')
    focusScan()
  } catch (error: any) {
    showFailToast(error?.message || '开始复核失败')
  }
}
...
async function scan() {
  ...
  loading.value = true
  try {
    order.value = await scanReview(id.value, code)
    scanCode.value = ''
    showSuccessToast('复核通过')
  } catch (error: any) {
    showFailToast(error?.message || '扫码复核失败')
  } finally {
    loading.value = false
    focusScan()
  }
...
function openException(line: ReviewLineDto) {
...
async function submitException() {
  ...
  loading.value = true
  try {
    order.value = await markReviewException(id.value, {
...
async function finish() {
  try {
    await showConfirmDialog({
      title: '完成出库复核',
      message: '全部库存已逐条复核通过，确认完成？',
      confirmButtonText: '完成复核',
    })
    loading.value = true
    await completeReview(id.value)
    showSuccessToast('复核已完成')
    router.replace({ name: 'PdaReviewList' })
  } catch (error: any) {
    if (error !== 'cancel') showFailToast(error?.message || '完成复核失败')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="h-screen flex flex-col bg-gray-100">
    <van-nav-bar class="custom-nav-bar shrink-0" left-arrow @click-left="router.back()">
      <template #title><span class="text-white font-bold">复核执行</span></template>
    </van-nav-bar>
    ...
    <div v-if="order" class="flex-1 overflow-y-auto pb-24">
      <section class="bg-white p-4 border-b border-gray-100">
        <div class="flex justify-between items-center">
          <div>
            <div class="text-xs text-slate-500">复核单号</div>
            <div class="text-lg font-black font-mono text-slate-900">{{ order.reviewNo }}</div>
          </div>
          <div class="text-right text-sm text-slate-500">
            <div>进度 <b class="text-sky-600">{{ order.reviewedLineCount }}/{{ order.totalLineCount }}</b></div>
            <div class="mt-1">{{ order.targetLocationCode }}</div>
          </div>
        </div>
      </section>

      <section v-if="editable" class="m-3 bg-slate-800 rounded-2xl p-4 shadow-md">
        <div class="text-white font-bold mb-2">扫描盘号、库存二维码或SN</div>
        ...
        <van-button
          v-if="order.status === ReviewStatus.Created"
          block
          round
      ...
      <section class="p-3 space-y-3">
        <article
          ...
          :class="line.status === ReviewLineStatus.Passed ? 'border-l-green-500' : line.status === ReviewLineStatus.Exception ? 'border-l-red-500' : 'border-l-amber-400'"
        >
    ...
    <footer v-if="order" class="fixed bottom-0 left-0 right-0 bg-white border-t p-3 z-30">
      <van-button block round type="success" :disabled="!canComplete" :loading="loading" @click="finish">
        完成复核
      </van-button>
    </footer>
    ...
    <van-popup v-model:show="exceptionVisible" position="bottom" round>
      <div class="p-5">
        <div class="text-lg font-black text-slate-800 mb-4">登记复核异常</div>
        <van-field v-model="exceptionType" label="异常类型" placeholder="标签不符、包装破损等" />
